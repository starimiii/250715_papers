...
class SmallPatchUnet2D(nn.Module):
    """专门针对小裁块优化的2D U-Net"""
    def __init__(self, in_channels=1, out_channels=1, channels=[32, 64, 128], n_angles=12):
    ...
    def _create_fc(self, input_features, device):
        ...
        return nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(0.1),
            nn.Linear(input_features//4, self.n_angles)
        ).to(device)
    ...
    def forward(self, x):
        ...
        for i, decoder in enumerate(self.decoders):
            ...
            x = decoder[1](x)  # CompactConvBlock2D
...
def create_model(in_channels=1, out_channels=1, channels=[16, 32, 64], n_angles=12, *args):
    return SmallPatchUnet2D(in_channels=in_channels, out_channels=out_channels, 
                 channels=channels, n_angles=n_angles)
